    ...
    <Stack fill vertical>
      <Stack.Item>
        <Tabs fluid>
          ...
          <Tooltip content="Добавить новое преступление или штрафы" position="bottom">
            <Tabs.Tab
  ...
  return (
    <Stack fill vertical>
      {!toDisplay.length ? (
        <Stack.Item>
          <NoticeBox>
            {tab === SECURETAB.Crimes ? 'Преступления' : 'Штрафы'} не найдены.
          </NoticeBox>
        </Stack.Item>
      ) : (
...
const CrimeDisplay = ({ item }: { item: Crime }) => {
  ...
  const { author, crime_ref, details, fine, name, paid, time, valid, voider } =
    item;
  const showFine = !!fine && fine > 0 ? `: ${fine} кр` : ': ВЫПЛАЧЕНО';
        ...
        <LabeledList>
          ...
          {!!fine && fine > 0 && (
            <>
              ...
              <LabeledList.Item color="good" label="Выплачено">
    ...
    <Stack fill vertical>
      ...
      <Stack.Item color="label">
        ...
        <TextArea
          ...
          placeholder="Введите некоторые подробности..."
        />
      </Stack.Item>
      <Stack.Item color="label">
        Штраф (оставьте поле пустым, чтобы арестовать)
        <RestrictedInput
          fluid
          value={crimeFine}
          maxValue={1000}
      ...
      <Stack.Item>
        <Button.Confirm
          ...
          icon="plus"
          onClick={createCrime}
          tooltip={!nameMeetsReqs ? 'ФИО должно быть не менее 3 символов.' : ''}
        >
          Создать
        </Button.Confirm>
      </Stack.Item>
