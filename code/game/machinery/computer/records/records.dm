...
/obj/machinery/computer/records/ui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		if("edit_field")
			...
			var/value = reject_bad_name(params["value"], allow_numbers = TRUE, max_length = MAX_BROADCAST_LEN, strict = TRUE, cap_after_symbols = FALSE) || "Неизвестно"
			investigate_log("[key_name(user)] изменил поле: \"[field]\" со значением: \"[target.vars[field]]\" на новое значение: \"[value]\"", INVESTIGATE_RECORDS)
		...
		if("expunge_record")
			...
			if(!is_station_level(z))
				balloon_alert(user, "вне диапазона!")
				return TRUE

			expunge_record_info(target)
			balloon_alert(user, "запись уничтожена")
			playsound(src, 'sound/machines/terminal/terminal_eject.ogg', 70, TRUE)
			investigate_log("[key_name(user)] уничтожил запись [target.name].", INVESTIGATE_RECORDS)

			return TRUE

		if("login")
			authenticated = secure_login(user)
			investigate_log("[key_name(user)] [authenticated ? "успешно вошёл" : "не смог войти"] в [declent_ru(NOMINATIVE)].", INVESTIGATE_RECORDS)
			return TRUE

		if("logout")
			balloon_alert(user, "выход из системы")
		...
		if("purge_records")
			...
			if(do_after(user, 5 SECONDS))
				...
				playsound(src, 'sound/machines/terminal/terminal_off.ogg', 70, TRUE)
				investigate_log("[key_name(user)] очистил все записи.", INVESTIGATE_RECORDS)
			else
...
/obj/machinery/computer/records/proc/secure_login(mob/user)
	...
	balloon_alert(user, "вход в систему")
