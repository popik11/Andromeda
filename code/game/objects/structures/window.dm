...
/obj/structure/window/attack_tk(mob/user)
	...
	user.visible_message(span_notice("Кто-то постучал в [src]."))
	...
	switch(state)
		if(WINDOW_SCREWED_TO_FRAME)
			...
			if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				...
				to_chat(user, span_notice("Вы открутили окно от рамы."))
		if(WINDOW_IN_FRAME)
			to_chat(user, span_notice("Вы начинаете прикручивать окно к раме..."))
			if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				state = WINDOW_SCREWED_TO_FRAME
				to_chat(user, span_notice("Вы прикрутили окно к раме."))
		if(WINDOW_OUT_OF_FRAME)
			if(anchored)
				to_chat(user, span_notice("Вы начинаете откручивать раму от пола..."))
				if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
					set_anchored(FALSE)
					to_chat(user, span_notice("Вы открутили раму от пола."))
			else
...
/obj/structure/window/crowbar_act(mob/living/user, obj/item/tool)
	...
	switch(state)
		if(WINDOW_IN_FRAME)
			to_chat(user, span_notice("Вы начинаете выдирать окно из рамы..."))
			if(tool.use_tool(src, user, 10 SECONDS, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				...
				to_chat(user, span_notice("Вы выдрали окно из рамы."))
		if(WINDOW_OUT_OF_FRAME)
...
/obj/structure/window/reinforced
	...
	desc = "Окно, усиленное металлическими стержнями."
	icon_state = "rwindow"
	reinf = TRUE
...
/obj/structure/window/reinforced/attackby_secondary(obj/item/tool, mob/user, list/modifiers, list/attack_modifiers)
	if(resistance_flags & INDESTRUCTIBLE)
		balloon_alert(user, "слишком прочно!")
	...
	switch(state)
		...
		if(RWINDOW_BOLTS_HEATED)
			if(tool.tool_behaviour == TOOL_SCREWDRIVER)
				user.visible_message(span_notice("[user] вгрызается в нагретые защитные винты и начинает их выкручивать..."),
										span_notice("Вы с силой вгрызаетесь в нагретые винты, и они начинают проворачиваться..."))
				if(tool.use_tool(src, user, 50, volume = 50))
					state = RWINDOW_BOLTS_OUT
					to_chat(user, span_notice("Винты выкручены, вокруг края панели образовался зазор."))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала нужно выкрутить защитные винты!"))

		if(RWINDOW_BOLTS_OUT)
			if(tool.tool_behaviour == TOOL_CROWBAR)
				user.visible_message(span_notice("[user] засовывает [tool] в зазор в раме и начинает поддевать..."),
										span_notice("Вы засовываете [tool] в зазор в раме и начинаете поддевать..."))
				if(tool.use_tool(src, user, 40, volume = 50))
					state = RWINDOW_POPPED
					to_chat(user, span_notice("Панель выскакивает из рамы, обнажая тонкие металлические прутья, которые, похоже, можно перекусить."))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала нужно поддеть зазор!"))

		if(RWINDOW_POPPED)
			if(tool.tool_behaviour == TOOL_WIRECUTTER)
				user.visible_message(span_notice("[user] начинает перекусывать прутья на [src]..."),
										span_notice("Вы начинаете перекусывать прутья на [src]"))
				if(tool.use_tool(src, user, 20, volume = 50))
					state = RWINDOW_BARS_CUT
					to_chat(user, span_notice("Панели падают, обнажая болты рамы."))
			else if (tool.tool_behaviour)
...
/obj/structure/window/proc/cool_bolts()
	if(state == RWINDOW_BOLTS_HEATED)
		state = RWINDOW_SECURE
		visible_message(span_notice("Винты на [src], похоже, остыли..."))
...
/obj/structure/window/reinforced/examine(mob/user)
	...
	switch(state)
		if(RWINDOW_SECURE)
			. += span_notice("Оно прикручено односторонними винтами, вам нужно <b>нагреть</b> их, чтобы была хоть какая-то возможность выкрутить их.")
		if(RWINDOW_BOLTS_HEATED)
			. += span_notice("Винты раскалены докрасна, и теперь вы, вероятно, сможете <b>выкрутить</b> их.")
		if(RWINDOW_BOLTS_OUT)
			. += span_notice("Винты удалены, виден небольшой зазор, в который можно <b>отогнуть</b> инструментом.")
		if(RWINDOW_POPPED)
			. += span_notice("Основная пластина окна выскочила из рамы, обнажая прутья, которые, похоже, можно <b>перекусить</b>.")
		if(RWINDOW_BARS_CUT)
			. += span_notice("Основное стекло можно легко сдвинуть, чтобы открыть <b>болты</b>, удерживающие раму.")
...
/obj/structure/window/plasma
	name = "plasma window"
	desc = "Окно из плазма-силикатного сплава и матрицы стержней. Выглядит безнадёжно прочным и скорее всего, практически огнеупорным."
...
/obj/structure/window/reinforced/plasma/plastitanium
	...
	desc = "Прочное на окно из сплава плазмы и титана."
...
/obj/structure/window/paperframe/examine(mob/user)
	. = ..()
	if(atom_integrity < max_integrity)
		. += span_info("Выглядит немного повреждённым, возможно, получится починить <b>бумагой</b>.")
...
/obj/structure/window/bronze
	name = "brass window"
	desc = "Тонкий как бумага лист просвечивающей, но усиленной латуни. Хотя нет, это просто слабая бронза!"
