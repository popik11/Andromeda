 ...
 */
/proc/power_failure()
	priority_announce("Обнаружена аномальная активность в энергосети [station_name()]. В качестве меры предосторожности питание станции будет отключено на неопределённый срок.", "Критический сбой энергосистем", ANNOUNCER_POWEROFF)
 ...
 */
/proc/power_restore()
	priority_announce("Питание на [station_name()] восстановлено. Приносим извинения за доставленные неудобства.", "Энергосистемы в норме", ANNOUNCER_POWERON)
	for(var/obj/machinery/power/apc/C as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/apc))
		if(C.cell && is_station_level(C.z))
...
/proc/power_restore_quick()
	priority_announce("Все СМЭСы на [station_name()] перезаряжены. Приносим извинения за доставленные неудобства.", "Энергосистемы в норме", ANNOUNCER_POWERON)
	var/list/all_smes = SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/smes)
	for(var/obj/machinery/power/smes/smes as anything in all_smes)
		if(!is_station_level(smes.z))
			continue
