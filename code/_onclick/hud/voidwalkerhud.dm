...
/atom/movable/screen/space_camo
	name = "переключение космического камуфляжа"
...
/atom/movable/screen/vomit_jump/examine(mob/user)
	...
	. += span_notice("Отслеживает все рвотные массы в туманности, созданные людьми, которых вы касались.")
	. += span_notice("ЛКМ во время космического прыжка перемещает к ближайшей рвоте в туманности (если есть).")
	. += span_notice("Текущее количество рвот в туманности: [vomits.len]")
...
/atom/movable/screen/vomit_jump/Click(location, control, params)
	...
	if(!istype(usr.loc, /obj/effect/dummy/phased_mob/space_dive))
		usr.balloon_alert(usr, "должен быть в режиме космического прыжка!")
		return

	var/list/vomits = get_valid_vomits()

	if(!vomits.len)
		usr.balloon_alert(usr, "нет рвот в туманности!")
		return
