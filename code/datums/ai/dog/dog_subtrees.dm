...
/datum/ai_behavior/find_hated_dog_target/setup(datum/ai_controller/controller, target_key, targeting_strategy_key)
	...
	for(var/mob/living/iter_living in oview(2, dog))
		...
		if(!isnull(dog.buckled))
			dog.audible_message(span_notice("[dog] рычит на [iter_living], но [dog.p_they()] [dog.p_are()] слишком уютно устроился, чтобы сдвинуться."), hearing_distance = COMBAT_MESSAGE_RANGE)
			continue
		if(!targeting_strategy.can_attack(dog, iter_living))
			continue

		dog.audible_message(span_warning("[dog] рычит на [iter_living], явно раздражен [iter_living.p_their()] присутствием."), hearing_distance = COMBAT_MESSAGE_RANGE)
