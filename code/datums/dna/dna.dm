...
/datum/dna/proc/update_instability(alert=TRUE)
	...
	if(holder)
		...
		if(alert)
			switch(stability)
				if(70 to 90)
					message = span_warning("Тебя знобит.")
				if(60 to 69)
					message = span_warning("Тебе холодно.")
				if(40 to 59)
					message = span_warning("Тебя тошнит.")
				if(20 to 39)
					message = span_warning("Кажется, будто кожа шевелится.")
				if(1 to 19)
					message = span_warning("Чувствуешь, как горят клетки.")
				if(-INFINITY to 0)
					message = span_boldwarning("ДНК разрушается, нужно срочно что-то сделать!")
		if(stability <= 0)
...
/mob/living/carbon/human/proc/something_horrible_mindmelt()
	if(!is_blind())
		...
		eyes.Remove(src)
		qdel(eyes)
		visible_message(span_notice("[src] поднимает взгляд, и его глаза растворяются!"), span_userdanger("Теперь я понимаю."))
