...
/datum/element/frozen/Attach(datum/target)
	...
	target_obj.name = "замороженный [target_obj.name]"
...
/datum/element/frozen/Detach(datum/source, ...)
	...
	obj_source.name = replacetext(obj_source.name, "замороженный ", "")
...
/datum/element/frozen/proc/shatter_on_landed(datum/target, datum/thrownthing/throwingdatum)
	...
	if(ismob(throwingdatum.thrower))
		log_combat(throwingdatum.thrower, target, "разбился", addition = "от броска, так как [target] был заморожен.")
	else
		log_combat(throwingdatum.thrower, target, "запустил", addition = "разбив его, так как он был заморожен.")
	obj_target.visible_message(span_danger("[obj_target] разлетается на миллион осколков!"))
	obj_target.obj_flags |= NO_DEBRIS_AFTER_DECONSTRUCTION	// отключить создание обломков
	obj_target.deconstruct(FALSE)			// вызов специализированного кода перед удалением - высвобождение газов и т.д.
