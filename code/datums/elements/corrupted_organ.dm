...
/datum/element/corrupted_organ/proc/on_removed(atom/organ, mob/living/remover, mob/living/carbon/loser)
	...
	if (prob(75))
		organ.AddComponent(\
			...
			aggro_radius = 4, \
			spawn_message = span_revenwarning("[organ] зловеще зависает в воздухе, пульсируя неестественной энергией!"), \
            despawn_message = span_revenwarning("[organ] безжизненно падает на землю."), \
		)
		return
	...
	organ.visible_message(span_revenwarning("[organ] взрывается всплеском тёмной энергии!"))
