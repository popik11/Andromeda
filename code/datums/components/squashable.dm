	...
	if(isliving(crossing_movable))
		...
		if(crossing_mob.mob_size > MOB_SIZE_SMALL && !(crossing_mob.movement_type & MOVETYPES_NOT_TOUCHING_GROUND))
			if(HAS_TRAIT(crossing_mob, TRAIT_PACIFISM))
				crossing_mob.visible_message(span_notice("[crossing_mob] аккуратно переступает через [parent_as_living]."), span_notice("Ты осторожно переступаешь через [parent_as_living], чтобы не навредить."))
				return
			if(should_squash)
				crossing_mob.visible_message(span_notice("[crossing_mob] раздавил [parent_as_living]."), span_notice("Ты раздавил [parent_as_living]."))
				Squish(parent_as_living)
			else
				parent_as_living.visible_message(span_notice("[parent_as_living] удалось избежать раздавливания."))
	else if(isstructure(crossing_movable))
		if(should_squash)
			crossing_movable.visible_message(span_notice("[parent_as_living] раздавлен под [crossing_movable]."))
			Squish(parent_as_living)
		else
			parent_as_living.visible_message(span_notice("[parent_as_living] удалось избежать раздавливания."))
