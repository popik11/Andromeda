...
/datum/component/usb_port/proc/on_examine(datum/source, mob/user, list/examine_text)
	...
	if (isnull(attached_circuit))
		examine_text += span_notice("На передней панели есть USB-порт.")
	else
		examine_text += span_notice("[attached_circuit.shell || attached_circuit] подключ[parent.p_them()] к [parent.p_them()] через USB-порт.")

/datum/component/usb_port/proc/on_examine_shell(datum/source, mob/user, list/examine_text)
	SIGNAL_HANDLER

	examine_text += span_notice("[source.p_They()] [source.p_are()] подключены к [parent] USB-кабелем.")
...
/datum/component/usb_port/proc/on_atom_usb_cable_try_attach(datum/source, obj/item/usb_cable/connecting_cable, mob/user)
	...
	if (isnull(connecting_cable.attached_circuit))
		if(user)
			connecting_cable.balloon_alert(user, "сначала подключите к оболочке")
		return COMSIG_CANCEL_USB_CABLE_ATTACK
