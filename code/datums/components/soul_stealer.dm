...
/datum/component/soul_stealer/proc/on_examine(datum/source, mob/user, list/examine_list)
	...
	examine_list += span_notice("Оно поглотит душу любого, кто падёт в бою перед ним.")

	var/num_souls = length(soulstones)
	switch(num_souls)
		if(0)
			examine_list += span_notice("Оно ещё не поглотило ни одной души.")
		if(1 to 9)
			examine_list += span_notice("Внутри заточено <b>[num_souls]</b> душ.")
		if(10 to INFINITY)
			examine_list += span_notice("Оно уже поглотило ошеломляющие <b>[num_souls]</b> душ! И жаждет ещё больше!")
