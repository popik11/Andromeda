...
/datum/component/unbreakable/proc/surge(mob/living/carbon/human/surged, new_stat)
	...
	surged.balloon_alert(surged, "ты отказываешься сдаваться!")//breaks balloon alert conventions by using a "!" for a fail message but that's okay because it's a pretty awesome moment
