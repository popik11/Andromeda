	...
	switch(punishment)
		if(PUNISHMENT_MURDER)
			if(escapee.stat != DEAD)
				escapee.investigate_log("был убит компонентом stationstuck.", INVESTIGATE_DEATHS)
			escapee.death()
		if(PUNISHMENT_GIB)
			escapee.investigate_log("был разорван компонентом stationstuck.", INVESTIGATE_DEATHS)
