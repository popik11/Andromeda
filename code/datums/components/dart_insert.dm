...
/datum/component/dart_insert/proc/on_preattack(datum/source, atom/target, mob/user, list/modifiers)
	...
	if(!istype(dart))
		return
	if(!dart.modified)
		to_chat(user, span_warning("Защитный колпачок не позволяет вставить [parent] в [dart]."))
		return COMPONENT_CANCEL_ATTACK_CHAIN
	if(HAS_TRAIT(dart, TRAIT_DART_HAS_INSERT))
		to_chat(user, span_warning("В [dart] уже что-то есть."))
		return COMPONENT_CANCEL_ATTACK_CHAIN
...
/datum/component/dart_insert/proc/add_to_dart(obj/item/ammo_casing/dart, mob/user)
	...
	if(user)
		if(!user.transferItemToLoc(parent_item, dart_projectile))
			return
		to_chat(user, span_notice("Вы вставляете [parent_item] в [dart]."))
	else
...
/datum/component/dart_insert/proc/remove_from_dart(obj/item/ammo_casing/dart, obj/projectile/projectile, mob/user)
	...
	if(user)
		...
		to_chat(user, span_notice("Вы достаёте [parent] из [dart]."))
...
/datum/component/dart_insert/proc/on_dart_examine_more(datum/source, mob/user, list/examine_list)
	var/obj/item/parent_item = parent
	examine_list += span_notice("Внутри виден [parent_item.name].")
