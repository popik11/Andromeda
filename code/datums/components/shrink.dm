...
/datum/component/shrink/Initialize(shrink_time)
	...
	if(isliving(parent_atom))
		...
		if(iscarbon(L))
			...
			C.visible_message(span_warning("Вещи [C] падают с [C.p_them()], пока [C.p_they()] уменьшаются!"),
			span_userdanger("Твои вещи падают, пока всё вокруг становится больше!"))
			if(ishuman(C))
	...
	parent_atom.visible_message(span_warning("[parent_atom] уменьшается до крошечного размера!"),
	span_userdanger("Всё вокруг становится больше!"))
