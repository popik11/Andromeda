...
/datum/brain_trauma/magic/lumiphobia
	name = "Люмифобия"
	desc = "У пациента наблюдается необъяснимая негативная реакция на свет."
	scan_desc = "гиперчувствительность к свету"
	gain_text = span_warning("Вы чувствуете тягу к темноте.")
	lose_text = span_notice("Свет больше не беспокоит вас.")
	/// Кулдаун для предотвращения спама предупреждениями
...
/datum/brain_trauma/magic/poltergeist
	name = "Полтергейст"
	desc = "Пациент, похоже, подвергается воздействию агрессивного невидимого существа."
	scan_desc = "паранормальная активность"
	gain_text = span_warning("Вы чувствуете злобное присутствие рядом с собой.")
	lose_text = span_notice("Вы чувствуете, как злобное присутствие исчезает.")
...
/datum/brain_trauma/magic/stalker
	...
	desc = "Пациента преследует фантом, которого видит только он."
	scan_desc = "экстрасенсорная паранойя"
	gain_text = span_warning("Вы чувствуете, что что-то хочет вас убить...")
	lose_text = span_notice("Вы больше не чувствуете пристального взгляда за спиной.")
	var/obj/effect/client_image_holder/stalker_phantom/stalker
	var/close_stalker = FALSE // Для сердцебиения
...
/datum/brain_trauma/magic/stalker/on_life(seconds_per_tick, times_fired)
	...
	if(get_dist(owner, stalker) <= 1)
		...
		owner.visible_message(span_warning("[owner] разрывается невидимыми когтями!"), span_userdanger("Призрачные когти разрывают ваше тело!"))
