...
/// Вызывается грязной едой. Заставляет вас рычать на людей и спонтанно кусать их.
/datum/disease/carpellosis
	name = "Карпеллёз"
	desc = "У вас внутри злой космическая карп."
	form = "Паразит"
	agent = "Карп Элла"
	cure_text = "Хлор"
	...
	switch(stage)
		if(2)
			if(SPT_PROB(1, seconds_per_tick) && affected_mob.stat == CONSCIOUS && affected_mob.get_organ_slot(ORGAN_SLOT_EXTERNAL_TAIL))
				to_chat(affected_mob, span_warning("Хочется помахать хвостом..."))
				affected_mob.emote("wag")
		if(3)
			if(SPT_PROB(1, seconds_per_tick) && affected_mob.stat == CONSCIOUS)
				to_chat(affected_mob, span_warning("Внезапно захотелось поплавать в космосе..."))
...
/datum/disease/carpellosis/cure(add_resistance = TRUE)
	...
	if(max_stage_reached && prob(ella_spawn_chance))
		to_chat(affected_mob, span_warning("Что-то выходит из вас!"))
...
/datum/disease/carpellosis/proc/gnash_someone()
	...
	if(isnull(target) || !affected_mob.get_bodypart(BODY_ZONE_HEAD)) // Need mouth to gnash
		to_chat(affected_mob, span_warning("Хочется кого-нибудь цапнуть...")) /// Не за жопу
		return
	to_chat(affected_mob, span_warning("[target.name] почему-то вызывает у вас злость..."))
	...
	if(affected_mob.Adjacent(target))
		...
		affected_mob.visible_message("кусает [target.name].", visible_message_flags = EMOTE_MESSAGE)
