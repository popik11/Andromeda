/datum/disease/pierrot_throat
	...
	desc = "Без лечения жертва, вероятно, доведёт окружающих до безумия."
	...
	switch(stage)
		if(1)
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_danger("Ты чувствуешь себя немного глупо."))
		if(2)
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_danger("Ты начинаешь видеть радуги."))
		if(3)
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_danger("Твои мысли прерывает громкое <b>ХОНК!</b>"))
		if(4)
