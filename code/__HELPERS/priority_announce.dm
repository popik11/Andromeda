...
/proc/priority_announce(text, title = "", sound, type, sender_override, has_important_message = FALSE, list/mob/players = GLOB.player_list, encode_title = TRUE, encode_text = TRUE, color_override)
	...
	switch(type)
		if(ANNOUNCEMENT_TYPE_PRIORITY)
			header = MAJOR_ANNOUNCEMENT_TITLE("Важное объявление")
			if(length(title) > 0)
	...
	if(isnull(sender_override) && players == GLOB.player_list)
		if(length(title) > 0)
			...
			GLOB.news_network.submit_article(text, "Объявление [command_name()]", NEWSCASTER_STATION_ANNOUNCEMENTS, null)

/proc/print_command_report(text = "", title = null, announce=TRUE)
	if(!title)
		title = "Секретное объявление [command_name()]"

	if(announce)
		priority_announce(
			text = "Отчёт был загружен и распечатан на всех коммуникационных консолях.",
...
/proc/level_announce(datum/security_level/selected_level, previous_level_number)
	...
	if(current_level_number > previous_level_number)
		title = "Уровень безопасности повышен до [current_level_name]:"
		message = selected_level.elevating_to_announcement
	else
		title = "Уровень безопасности понижен до [current_level_name]:"
