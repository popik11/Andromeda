...
/obj/machinery/vending/screwdriver_act(mob/living/user, obj/item/attack_item)
	if(anchored)
		...
		to_chat(user, span_warning("Вы должны сначала закрепить [src]."))
		return ITEM_INTERACT_FAILURE
...
/obj/machinery/vending/proc/canLoadItem(obj/item/loaded_item, mob/user, send_message = TRUE)
	...
	if(!length(loaded_item.contents) && (products[loaded_item.type] || premium[loaded_item.type] || contraband[loaded_item.type]))
		return TRUE
	if(send_message)
		to_chat(user, span_warning("[src] не принимает [loaded_item]!"))
	return FALSE
...
/obj/machinery/vending/proc/loadingAttempt(obj/item/inserted_item, mob/user)
	...
	if(!canLoadItem(inserted_item, user))
		to_chat(user, span_warning("[src] не принимает [inserted_item]!"))
		return FALSE

	to_chat(user, span_notice("Вы вставляете [inserted_item] в входной отсек [src]."))
	for(var/datum/data/vending_product/product_datum in product_records + coin_records + hidden_records)
		if(inserted_item.type == product_datum.product_path)
			if(product_datum.amount == product_datum.max_amount)
				to_chat(user, span_warning("нет места для дополнительных [product_datum.category || "Продуктов"]!"))
				return FALSE

			if(!user.transferItemToLoc(inserted_item, src))
				to_chat(user, span_warning("[inserted_item] застрял в вашей руке!"))
				return FALSE
...
/obj/machinery/vending/item_interaction(mob/living/user, obj/item/attack_item, list/modifiers)
	...
	if(compartmentLoadAccessCheck(user) && !user.combat_mode)
		...
		if (!is_operational)
			to_chat(user, span_warning("[src] не отвечает."))
		else if(istype(attack_item, /obj/item/storage/bag)) //обычно подносы
			...
			for(var/obj/item/the_item in storage_item.contents)
				if(loadingAttempt(the_item, user))
					...
					denied_items++
			if(denied_items)
				to_chat(user, span_warning("[src] отказывается принимать некоторые предметы!"))
			if(loaded)
				to_chat(user, span_notice("Вы вставляете [loaded] блюд в отсек [src]."))
				return ITEM_INTERACT_SUCCESS
		else
...
/obj/machinery/vending/proc/post_restock(mob/living/user, restocked)
	...
	to_chat(user, span_notice("Вы загрузили [restocked] предметов в [src][credits_contained > 0 ? " и получаете [credits_contained] кредитов." : "."]"))
	var/datum/bank_account/cargo_account = SSeconomy.get_dep_account(ACCOUNT_CAR)
	cargo_account.adjust_money(round(credits_contained * 0.5), "ТоргоМат: Пополнение запасов")
...
/obj/machinery/vending/proc/freebie(freebies)
	...
	visible_message(span_notice("[src] выдаёт [freebies > 1 ? "несколько бесплатных плюшек" : "бесплатную плюшку"][credits_contained > 0 ? " и немного кредитов" : ""]!"))
...
/obj/machinery/vending/attack_tk_grab(mob/user)
	to_chat(user, span_warning("[src], кажется, сопротивляется вашему ментальному захвату!"))
