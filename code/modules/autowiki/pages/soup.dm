...
/datum/autowiki/soup/generate()
	...
	for(var/soup_recipe_type in subtypesof(/datum/chemical_reaction/food/soup))
		...
		if(istype(soup_recipe, /datum/chemical_reaction/food/soup/custom))
			...
			template_list["name"] = "Особый суп"
			template_list["taste"] = "Что добавите, то и почувствуете."
			template_list["foodtypes"] = "Зависит от ингредиентов."
			template_list["description"] = "Особый рецепт супа, позволяющий бросить в кастрюлю всё что угодно."
