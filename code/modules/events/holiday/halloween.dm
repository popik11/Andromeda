...
/datum/round_event/spooky/announce(fake)
	priority_announce(pick("ПОТРЯСИ МОИ КОСТИ!", "АТТРАКЦИОН НИКОГДА НЕ ЗАКАНЧИВАЕТСЯ!", "ВЫСКАКИВАЕТ СКЕЛЕТ!", "ЖУТКИЕ СТРАШНЫЕ СКЕЛЕТЫ!", "ЧЛЕНЫ ЭКИПАЖА, БЕРЕГИТЕСЬ, ВАС ЖДЕТ ИСПУГ!"), "ЗВОНОК ИДЕТ ИЗНУТРИ ДОМА")
