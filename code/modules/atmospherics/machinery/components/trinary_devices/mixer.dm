/obj/machinery/atmospherics/components/trinary/mixer
	...
	name = "gas mixer"
	desc = "Очень полезно для смешивания газов."
...
/obj/machinery/atmospherics/components/trinary/mixer/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("node1")
			...
			investigate_log("был установлен на [100 * node1_concentration] % на узле 1 пользователем [key_name(usr)]", INVESTIGATE_ATMOS)
			. = TRUE
		if("node2")
			var/value = text2num(params["concentration"])
			adjust_node1_value(100 - value)
			investigate_log("был установлен на [100 * node2_concentration] % на узле 2 пользователем [key_name(usr)]", INVESTIGATE_ATMOS)
			. = TRUE
