...
/mob/eye/blob/process()
	if(!blob_core)
		...
		priority_announce("Биологическая угроза достигла критической массы. Потеря станции неминуема.", "Биологическая угроза")
	...
	if(announcement_time && (world.time >= announcement_time || blobs_legit.len >= announcement_size) && !has_announced)
		priority_announce("Подтверждена вспышка биологической угрозы 5 уровня на борту [station_name()]. Весь персонал должен сдержать распространение.", "Биологическая угроза")
		has_announced = TRUE
