...
/obj/machinery/computer/emergency_shuttle/ui_act(action, params, datum/tgui/ui)
	...
	if((old_len != authorized.len) && !ENGINES_STARTED)
		...
		if(authorized.len && remaining)
			minor_announce("[remaining] авторизация необходима до тех пор, пока шаттл не будет запущен досрочно", null, alert)
		if(repeal)
			minor_announce("Разрешение на досрочный запуск отозвано, [remaining] авторизаций осталось")
...
/obj/machinery/computer/emergency_shuttle/process()
	...
	if((authorized.len >= auth_need) || (obj_flags & EMAGGED))
		...
		minor_announce("Аварийный шаттл стартует в течении \
