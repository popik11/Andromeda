...
#define TEXT_NO_TASTE "что-то неописуемое"
...
/proc/generate_reagents_taste_message(list/reagent_list, mob/living/taster, minimum_percent)
	...
	if(LAZYLEN(strong))
		out += "сильный вкус [english_list(strong, TEXT_NO_TASTE)]"
	if(LAZYLEN(mild))
		// Префикс "лёгкий " если есть сильные вкусы, чтобы не казалось сильным вкусом
		out += "[LAZYLEN(strong) ? "лёгкий " : ""][english_list(mild, TEXT_NO_TASTE)]"
	if(LAZYLEN(hint))
		out += "лёгкое послевкусие [english_list(hint, TEXT_NO_TASTE)]"
