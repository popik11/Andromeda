...
/mob/living/carbon/soundbang_act(intensity = 1, stun_pwr = 20, damage_pwr = 5, deafen_pwr = 15)
	...
	if(ears && (deafen_pwr || damage_pwr))
		...
		if(ears.damage >= 15 && prob(ears.damage - 5))
			to_chat(src, span_userdanger("Ты ничего не слышишь!"))
			// Полная глухота, требуется лечение
			ears.set_organ_damage(ears.maxHealth)
		to_chat(src, span_warning("В ушах начинает [ears.damage >= 15 ? " очень сильно звенеть!":"звенеть!"]"))
