...
/mob/living/carbon/human/get_visible_name(add_id_name = TRUE, force_real_name = FALSE, declent)
	...
	// Мы просто какой-то неизвестный парень
	if(HAS_TRAIT(src, TRAIT_UNKNOWN_APPEARANCE) || HAS_TRAIT(src, TRAIT_INVISIBLE_MAN))
		return "Неизвестный"

	// У нас есть лицо и ID
	if(face_name && id_name)
		var/normal_id_name = get_id_name("") // нужно проверить базовое имя ID, чтобы избежать "Джон (как Капитан Джон)"
		if(normal_id_name == face_name)
			return id_name // (это превращает "Джон" в "Капитан Джон")
		if(add_id_name)
			return "[face_name] (как [id_name])"

	// Просто идем по списку того, что мы записали
