...
/obj/machinery/cryopod
	...
	icon = 'modular_andromeda/icons/obj/machines/cryogenics.dmi'
...
/obj/machinery/cryopod/proc/find_control_computer(force = FALSE)
	...
	if(isnull(control_computer_weakref))
		log_admin("Криопод в [get_area(src)] не смог найти управляющий компьютер!")
		message_admins("Криопод в [get_area(src)] не смог найти управляющий компьютер!")
		return FALSE
...
/obj/machinery/cryopod/proc/put_other_inside(mob/target, mob/user)
	...
	if(target.client)
		...
		message_admins("[key_name_admin(user)] пытается переместить [key_name_admin(target)] в криопод, пока к нему все еще подключен клиент")
		log_admin("[key_name(user)] пытается переместить [key_name(target)] в криопод, пока к нему все еще подключен клиент")
		return
	...
	if(target.mind?.assigned_role.req_admin_notify)
		...
		message_admins("[key_name_admin(user)] пытается переместить [key_name_admin(target)] с важной должностью [target.mind.assigned_role.title] в криопод")
		log_admin("[key_name(user)] пытается переместить [key_name(target)] с важной должностью [target.mind.assigned_role.title] в криопод")
	...
	if(length(target_special_roles))
		...
		message_admins("[key_name_admin(user)] пытается переместить в криопод [key_name_admin(target)] с особыми ролями: [special_roles_text]")
		log_admin("[key_name(user)] пытается переместить в криопод [key_name(target)] с особыми ролями: [special_roles_text]")
...
/obj/machinery/cryopod/proc/put_self_inside(mob/target)
	...
	if(target.mind?.assigned_role.req_admin_notify)
		...
		message_admins("[key_name_admin(target)] пытается войти в криопод с важной должностью [target.mind.assigned_role.title]")
		log_admin("[key_name(target)] пытается войти в криопод с важной должностью [target.mind.assigned_role.title]")
	...
	if(length(target_special_roles))
		...
		var/special_roles_text = english_list(target_special_roles)
		message_admins("[key_name_admin(target)] пытается войти в криопод с особыми ролями: [special_roles_text]")
		log_admin("[key_name(target)] пытается войти в криопод с особыми ролями: [special_roles_text]")
