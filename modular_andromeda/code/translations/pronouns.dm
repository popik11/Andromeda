// MARK: Вспомогательные процедуры
...
/proc/declension_ru(num, single_name, double_name, multiple_name)
	...
	if(((num % 10) == 1) && ((num % 100) != 11)) // 1, не 11
		return single_name
	if(((num % 10) in 2 to 4) && !((num % 100) in 12 to 14)) // 2, 3, 4, не 12, 13, 14
		return double_name
...
/**
 * Заменяет в сообщении части `%(ЕД,МН)%` или `%(МУЖ,ЖЕН,СР,МН)%` согласно полу пользователя.
 * Используйте `*` для явного пропуска варианта: `%(*,ЖЕН,*,МН)%`.
 *
 * Пример: var/death_message = genderize_decode(user, "изда%(ёт,ют)% тихий гортанный звук, зелёная кровь пузырится из %(его,её,его,их)% пасти...")
 *
 * Аргументы:
 * * user - Персона, чьи местоимения используются.
 * * msg - Строка для обработки.
 *
 * Возвращает обработанную строку msg.
 */
/proc/genderize_decode(atom/target, msg)
	if(!isatom(target) || !istext(msg))
		stack_trace("Недопустимые аргументы в процедуре genderize_decode.")
	while(TRUE)
...
//////////////////////////////
// MARK: Местоимения мобов
//////////////////////////////
// Mobs (и атомы, но атомы не имеют особого значения, напишите свои собственные переопределения процедур) также имеют пол!
...
//////////////////////////////
// MARK: Человеческие местоимения
//////////////////////////////
// Людям нужно особое отношение, потому что их пол может быть скрыт.
