...
ADMIN_VERB(ru_names_review_panel, R_ADMIN, "Обзор Ru Names", "Показывает предлагаемые игроком значения для ru-names", ADMIN_CATEGORY_MAIN)
...
/datum/ru_names_review_panel/proc/approve_entry(entry_id)
	...
	if(!json_data[entry_id])
		to_chat(usr, span_notice("Не удалось найти запись [entry_id]. Возможно, она уже одобрена или отклонена."))
		return
	...
	var/message = "одобряется [suggested_list] для [data["atom_path"]]"
	// Здесь мы отправляем сообщение в Discord.
	var/list/webhook_data = list(
		"title" = data["atom_path"],
		"fields" = list(
			list(
				"name" = "Предлагаемый список:",
				"value" = "\
	...
	to_chat(usr, span_notice("Запись [entry_id] одобрена."))
...
/datum/ru_names_review_panel/proc/deny_entry(entry_id)
	...
	if(!length(json_data))
		return
	if(!json_data[entry_id])
		to_chat(usr, "Не удалось найти запись [entry_id]. Возможно, она уже одобрена или отклонена.")
		return
	var/list/data = json_data[entry_id]
	var/suggested_list = "RU_NAMES_LIST_INIT(\"[data["suggested_list"]["base"]]\", \"[data["suggested_list"][NOMINATIVE]]\", \"[data["suggested_list"][GENITIVE]]\", \"[data["suggested_list"][DATIVE]]\", \"[data["suggested_list"][ACCUSATIVE]]\", \"[data["suggested_list"][INSTRUMENTAL]]\", \"[data["suggested_list"][PREPOSITIONAL]]\")"
	var/message = "одобряется [suggested_list] для [data["atom_path"]]"
	...
	var/log_text = "[key_name_and_tag(usr)] [message]"
	logger.Log(LOG_CATEGORY_RU_NAMES_SUGGEST, log_text)
	to_chat(usr, span_notice("Запись [entry_id] отклонена."))
...
/datum/ru_name_suggest_panel/New(new_data)
	if(!length(new_data))
		CRASH("Панель «Подсказка имени» создана без данных!")
	ru_name_data = list(
		"ckey" = new_data["ckey"],
