...
/atom
	// modular_andromeda\code\_DEFINES\pronouns.dm for more info
...
/proc/ru_names_list(base, nominative, genitive, dative, accusative, instrumental, prepositional, gender)
	if(!base || !nominative || !genitive || !dative || !accusative || !instrumental || !prepositional)
		CRASH("ru_names_list() получил неполный список!")
...
/datum/proc/declent_ru(declent = NOMINATIVE)
	CRASH("Использовался нереализованный proc/declent_ru()")
...
/// Используется для получения начальных значений, например для рецептов, где итоговый атом ещё не создан. Может возвращать null - используйте var/override_backup для гарантированного получения значения.
