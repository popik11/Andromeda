GLOBAL_VAR_INIT(blooper_allowed, TRUE) // Для администраторов

// мы позволяем боргам тоже иметь немного барков
/mob/living/silicon/Login()
	// Это единственная найденная функция, которая обновляет клиент для боргов.
...
/datum/admins/proc/toggleblooper()
	set category = "Server"
	set desc = "Переключить раздражающие барки."
	set name = "Переключить Барки"
	toggle_blooper()
	log_admin("[key_name(usr)] переключил Барки.")
	message_admins("[key_name_admin(usr)] переключил Барки.")
	SSblackbox.record_feedback("nested tally", "admin_toggle", 1, list("Переключить Барки", "[GLOB.blooper_allowed ? "Включено" : "Отключено"]")) // Если вы копируете это, убедитесь, что 4-й параметр уникален для новой процедуры!
...
/proc/toggle_blooper(toggle = null)
	...
	to_chat(world, "<span class='oocplain'><B>Барки глобально [GLOB.blooper_allowed ? "включены" : "отключены"].</B></span>")
...
/datum/preference_middleware/blooper
	/// Перезарядка на запрос предпросмотра Барка.
...
/// Могу ли я слышать блипы всех остальных?
...
/// Могу ли я иметь ползунок для регулировки громкости лаев?
...
/// Это было украдено из сборки Splurt >:3 и у fluffySTG!! ниихихихихи!~ // Rewokin: Долбаёбы из пиндосии срут в коде своим фурри ерп хуйнёй в комитах.
/datum/blooper
	var/name = "None"
	var/id = "No Voice"
	var/soundpath
	...
	// Переменные скорости. Скорость определяет количество символов, необходимых для каждого блипера, с меньшими скоростями, более быстрыми и с большей плотностью блиперов
	var/minspeed = BLOOPER_DEFAULT_MINSPEED
	var/maxspeed = BLOOPER_DEFAULT_MAXSPEED

	// Переменные видимости. Независимо от установленных ниже значений, их все равно можно получить через админку и генетику. Правило веселья.
	var/list/ckeys_allowed
	var/ignore = FALSE // Если TRUE - только для админов
